...
function buildNav(members) {
  ...
  nav = nav.concat(buildMemberNav(members.namespaces, "Categories", seen, linkto))
  ...
  if (items && items.length) {
    ...
    items.forEach(function(item) {
      ...
      if (!hasOwnProp.call(item, "longname")) {
        list += buildNavItem(linkfoFn('', item.name))
        return
      }
      ...
      if (!hasOwnProp.call(itemsSeen, item.longname)) {
        ...
        if (methods.length) {
          ...
          methods.forEach(function(method) {
            ...
            sublist += buildNavItem(buildNavType(method.kind, linkto(method.longname, method.name)))
          })
          sublist += '</ul>'
        }

        if (itemHeading === 'Tutorials') {
          list += buildNavItem(linktoFn(item.longname, displayName))
        } else {
          list += buildNavHeading(buildNavType(item.kind, linktoFn(item.longname, displayName)), sublist)
        }
    ...
    list += '</ul>'
    // nav = buildNavHeading(itemHeading, list)
    nav = list
  }
 ...
 * @return {String}
 */
function buildNavHeading (content, list = '') {
  return [
    '<li>',
